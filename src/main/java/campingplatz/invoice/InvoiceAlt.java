...
public class InvoiceAlt {
 
 private BaseFont bfBold;
 private BaseFont bf;
 private int pageNumber = 0;
  
 public static void createInvoice(HttpServletResponse response) {
    InvoiceAlt generateInvoice = new InvoiceAlt();
    generateInvoice.createPDF(response);
   ...
   private void generateLayout(Document doc, PdfContentByte cb)  {
    ...
    try {
     ...
     //add the images
     Image companyLogo = Image.getInstance("./src/main/resources/invoice/campsite.png");
   ...
   private void generateDetail(Document doc, PdfContentByte cb, int index, int y)  {
    ...
    try {
     ...
     double price = 1.11;
     //Double.valueOf(df.format(Math.random() * 10));
   ...
   private void printEndPage(PdfContentByte cb){
   
   
    cb.beginText();
    cb.setFontAndSize(bfBold, 8);
    cb.showTextAligned(PdfContentByte.ALIGN_RIGHT, "Page No. " + (pageNumber+1), 500, 50, 0);
    cb.endText(); 
